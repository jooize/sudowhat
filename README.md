...
**Redirecting sudo's stderr suppresses the verification code.** The code is written to sudo's **stderr** — chosen so it survives the common stdout redirects that would otherwise swallow it: `sudo cmd > file` and the canonical `sudo tee file > /dev/null` heredoc idiom both move stdout elsewhere, but the code still reaches your terminal. A caller that redirects sudo's *stderr* instead — `2>/dev/null`, or `&>` / `>&` capturing both streams — does remove the code from view. The exact-command display inside the Touch ID sheet is unaffected by any redirection; that is the redirect-proof signal. So treat the code as *positive confirmation* (a matching code means the prompt is the sudo you just launched), never as a gate: its **absence** is ambiguous — an innocent stderr redirect looks identical to a prompt you did not start — so a missing code is a cue to read the command in the sheet, not to approve reflexively.
