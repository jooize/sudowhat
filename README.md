...
| `authCacheMinutes` | minutes \| `null` (`0`) | sudo's credential cache. `0` = every command re-prompts; `null` = defer to your sudoers' own `timestamp_timeout` (writes no file). Never sets `timestamp_type=global`. |
