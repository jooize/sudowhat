    ...
    darwinConfigurations."<host>" = nix-darwin.lib.darwinSystem {
      modules = [
        sudowhat.darwinModules.default
        {
          ...
          # Show user/directory/input on the terminal before auth,
...
/etc/sudo.conf
  └── Plugin sudowhat_audit_plugin /usr/local/libexec/sudo/sudowhat_audit.so
      ...
      • writes user / directory / input to /dev/tty — every path,
...
as macOS — prints `user / directory / input` to your terminal *before* sudo's
